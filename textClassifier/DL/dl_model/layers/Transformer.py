...
class Transformer(object):
    ...
    def __init__(self, config, wordEmbedding):
        ...
        self.embeddedPosition = tf.placeholder(tf.float32, [None, self.config.sequenceLength, self.config.sequenceLength], name="embeddedPosition")
        # 定义l2损失
        ...
        with tf.name_scope("transformer"):
            for i in range(self.config.model.numBlocks):
                with tf.name_scope("transformer-{}".format(i + 1)):
                    # 维度[batch_size, sequence_length, embedding_size]
                    multiHeadAtt = self._multiheadAttention(
                        rawKeys=self.inputX, 
                        queries=self.embeddedWords,
                        keys=self.embeddedWords
                    )
                    # 维度[batch_size, sequence_length, embedding_size]
                    self.embeddedWords = self._feedForward(
                        multiHeadAtt, 
                        [config.model.filters, config.embeddingSize + config.sequenceLength]
                    )
            outputs = tf.reshape(
                self.embeddedWords, 
                [-1, config.sequenceLength * (config.embeddingSize + config.sequenceLength)]
            )
